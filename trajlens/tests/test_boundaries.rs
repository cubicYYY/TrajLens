...
async fn test_build_with_consensus_boundaries() {
    ...
    let boundaries = vec![2, 18, 30, 34, 40, 46, 51, 56, 62, 66, 72, 77, 84, 89, 92, 97, 100, 103, 106];
    
    let tree = goal_tree::build_with_boundaries(
        &trajectory, &boundaries, "bedrock/us.anthropic.claude-sonnet-4-6"
    ).await.expect("build_with_boundaries failed");
    
    println!("Nodes: {}", tree.nodes.len());
    for n in &tree.nodes {
        let sr = n.step_range;
        println!("  {:6} [{:7}] [{:7}] steps {:3}-{:<3} | {}", 
            n.node_id, format!("{:?}", n.goal_type).to_lowercase(), 
    ...
    std::fs::write("/data4/ye472/TrajLens/output/variance_test/consensus_goal_tree.igr.toml", &igr).unwrap();
    
    assert!(true, "Test passed — check output/variance_test/consensus_goal_tree.igr.toml");
}
